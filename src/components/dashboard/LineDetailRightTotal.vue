      ...
      <div id="tab-panel1" class="tabpanel active" role="tabpanel" aria-labelledby="tab1">
        <table>
          ...
          <tr>
            ...
            <td><p>{{Math.floor(data1['monthlyArcTime']/3600)}}</p></td>
            <td><p>{{Math.floor(data1['monthlyWorkTime']/3600)}}</p></td>
          </tr>
          <tr>
            <td>
              <p>
          ...
          <tr>
              <td>
                  <p>
                      <em>실시간</em>
                      <span>{{$check_undefined_and_to_fixed(arc_real_time_robot_data['arcRate'], 1)}}%</span>
                  </p>
              </td>
              <td><p>{{$n_to_0n(Math.floor(arc_real_time_robot_data['arcTime']/3600))}} : {{$n_to_0n(arc_real_time_robot_data['arcTime']%60)}}</p></td>
              <td><p>{{$n_to_0n(arc_real_time_robot_data['workHours'])}} : {{$n_to_0n(arc_real_time_robot_data['workTime']%60)}}</p></td>
          </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="bot_table">
      <table>
        ...
        <tr>
          ...
          <td><p>{{Math.floor(data2['monthlyWorkTime']/3600)}}</p></td>
        </tr>
        <tr>
          <td>
            <p>
          ...
          <td><p>{{Math.floor(data2['weeklyWorkTime']/3600)}}</p></td>
        </tr>
        <tr>
          <td>
            <p>
        ...
        <td>
            <p>
                <em>실시간</em>
                <span>{{$check_undefined_and_to_fixed(arc_real_time_manual_data['arcRate'], 1)}}%</span>
            </p>
        </td>
        <td><p>{{$n_to_0n(Math.floor(arc_real_time_manual_data['arcTime']/3600))}} : {{$n_to_0n(arc_real_time_manual_data['arcTime']%60)}}</p></td>
        <td><p>{{$n_to_0n(arc_real_time_manual_data['workHours'])}} : {{$n_to_0n(arc_real_time_manual_data['workTime']%60)}}</p></td>
        </tr>
        </tbody>
      </table>
    </div>
...
export default {
  ...
  props: {
    data1: Object,
    data2: Object,
    arc_real_time_robot_data: Object,
    arc_real_time_manual_data: Object,
  ...
  mounted() {
    console.log('[RightTotal] mounted', this.outer_check_data, this.robot_cnt_data)
...
.right{width: 450px;display: inline-block;vertical-align: top;padding-left: 40px; padding-top:44px;}
...
.right table tbody p{border-right: 1px solid #707070;margin: 5px 0;line-height: 1; font-size: 16px; font-family: 'Noto Sans KR';color: black;}
...
.right table tbody p{font-weight: 600;}
.right table tbody td.big span{font-size: 32px;width: 55px;display: inline-block;text-align: right;box-sizing: border-box;vertical-align: middle;line-height: 1;}
...
.right .table_scr{height: 116px;overflow-y:scroll;}
#tab-panel2 .table_scr{height:204px;border: 2px solid #002f63;overflow-y: scroll;}
